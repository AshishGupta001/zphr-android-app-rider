...
public class BusinessProfileListFragment extends BaseFragment {
    ...
    private void addFareDetailRow(HashMap<String, String> data, boolean isLast) {
        ...
        mainview.setOnClickListener(view -> businessProfileActivity.OpenProfileIntroScreen(data, true));


        String vImage = data.get("vImage");
        if (!vImage.equalsIgnoreCase("")) {
            new LoadImage.builder(LoadImage.bind(vImage), profileImage).setErrorImagePath(R.mipmap.ic_no_icon).setPlaceholderImagePath(R.mipmap.ic_no_icon).build();
