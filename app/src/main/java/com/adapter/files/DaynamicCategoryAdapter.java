...
public class DaynamicCategoryAdapter extends RecyclerView.Adapter<DaynamicCategoryAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        new LoadImage.builder(LoadImage.bind(listData.get(position).get("vBgImage")), holder.categoryBG).build();
        holder.categoryTxt.setText(listData.get(position).get("vCategoryName"));
        ...
        String img = listData.get(position).get("vIconImage");
        if (Utils.checkText(img)) {

            if (generalFunc.isRTLmode()) {

                new LoadImage.builder(LoadImage.bind(img), holder.categoryRTLImgView).build();
                ...
                new LoadImage.builder(LoadImage.bind(img), holder.categoryImgView).build();
